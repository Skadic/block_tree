...
public:
  ...
  std::vector<RecursiveBitBlockTree<size_type, recursion_level - 1>*>
      block_tree_types_;
  ...
  bool access(const size_type bit_index) const {
    ...
    for (size_t i = 0; i < height(); i++) {
      ...
      if (!is_internal.access(block_index)) {
        // If this block is not internal, go to its pointed-to block
        const size_t back_block_index = is_internal.rank0(block_index);
      ...
      off %= block_size;
      block_index = is_internal.rank1(block_index) * tau_ + child;
    }
    const uint8_t byte =
...
public:
  [[nodiscard("select result discarded")]] size_t select1(size_t rank) const {
    ...
    if (!top_is_internal[current_block]) {
      const size_t back_block_index = top_is_internal.rank0(current_block);
    ...
    while (level < height()) {
      ...
      const auto& prev_is_internal = *block_tree_types_[level - 1];
      ...
      current_block = prev_is_internal.rank1(current_block) * tau_;
      block_size /= tau_;
      ...
      if (!is_internal.access(current_block)) {
        size_t back_block_index = is_internal.rank0(current_block);
    ...
    current_block = block_tree_types_[level - 1]->rank1(current_block) * tau_;
  ...
  rank1(const size_type bit_index) const {
    ...
    if (!top_is_internal[block_index]) {
      ...
      const size_t back_block_index = top_is_internal.rank0(block_index);
      rank -= pointer_prefix_one_counts_[0][back_block_index];
    ...
    block_offset %= block_size;
    block_index = top_is_internal.rank1(block_index) * tau_ + child;
...
protected:
  ...
  size_type bit_rank_block(size_type level, size_type block_index) {
    ...
    if (is_internal[block_index]) {
      const size_type internal_index = is_internal.rank1(block_index);
      ...
      const size_type back_block_index = is_internal.rank0(block_index);
...
class RecursiveBitBlockTree<size_type, 0> {
public:
  /// If this is true, then the only levels of the tree start to be
  ///   included starting at the first level that contains a back block
  ///
  ...
  bool access(const size_type bit_index) const {
    ...
    int64_t block_size = height() == 0 ? leaf_size : block_size_lvl_[0];
    int64_t block_index = byte_index / block_size;
    int64_t off = byte_index % block_size;
    for (size_t i = 0; i < height(); i++) {
      const auto& is_internal = *block_tree_types_[i];
