...
protected:
  ...
  /// @param[in] bv_length The number of bits in the input bit vector.
  ...
  void calculate_padding(int64_t& padding,
    ...
    int64_t h = 1;
    // Bit size of the blocks on the current level (starting at the leaf level)
    blk_size = tau_;
    // While the tree does not cover the entire text, add a level
    while (tmp_padding < bv_length) {
