...
class RecursiveBitBlockTreeSharded
  ...
  ///   parameters
  using RabinKarp = MersenneRabinKarp<uint8_t, size_type, PRIME_EXPONENT>;
...
#  pragma omp parallel default(none) num_threads(threads)                      \
...
#endif
    {
      ...
      if constexpr (use_hash == UseHash::RABIN_KARP) {
        ...
        const uint64_t HASH_MASK = HASH_MASKS[pair_size];
        for (size_t i = start; i < end; ++i) {
...
#  pragma omp parallel default(none) num_threads(threads)                      \
...
#endif
    {
      ...
      if constexpr (use_hash == UseHash::RABIN_KARP) {
        ...
        const uint64_t HASH_MASK = HASH_MASKS[block_size];
        for (size_t i = start; i < end; ++i) {
          const size_t block_start = block_starts[i];
          const uint8_t* block_start_ptr = text.data() + block_start;
          const uint64_t hash_value =
...
#  endif
  ...
  scan_windows_in_block_identity(const std::span<const uint8_t>& text,
    ...
    const uint64_t HASH_MASK = HASH_MASKS[level_data.block_size];
    const uint8_t* block_start_ptr = text.data() + block_start;
  ...
  void make_tree(const std::span<const uint8_t> text,
    ...
    if (found_back_block) {
      ...
      this->block_tree_types_.back()->add_bit_rank_support();
  ...
  void make_tree_level(std::vector<LevelData>& levels,
    ...
    BitVector is_internal(new_size);
    ...
    sdsl::util::bit_compress(*pointers);
    sdsl::util::bit_compress(*offsets);
    auto* bt = new RecursiveBitBlockTreeSharded<size_type, recursion_level - 1>(
        is_internal,
        this->tau_,
        this->s_,
        this->max_leaf_length_,
...
public:
  ...
  ~RecursiveBitBlockTreeSharded() {
    for (auto& bt : this->block_tree_types_) {
      delete bt;
    }
...
class RecursiveBitBlockTreeSharded<size_type, 0>
    : public RecursiveBitBlockTree<size_type, 0> {
  using Clock = std::chrono::high_resolution_clock;
  using TimePoint = Clock::time_point;
...
#  endif
  ...
  void make_tree(const std::span<const uint8_t> text,
    ...
    if (found_back_block) {
      ...
      sdsl::util::bit_compress(*pointers);
      sdsl::util::bit_compress(*offsets);
      // We cannot "release()" is_internal yet.
      // If this is the only level of the tree, we need it later when
      // constructing the leaf string
    ...
    // Release the top-level bitvector,
    // since now we definitiely don't need it anymore
    levels[0].is_internal.release();
    this->amount_of_leaves = leaf_count;
    this->compress_leaves();
  }
